            ...
            if(pCommand->GetAccessLevel() >= m_AccessLevel)
			{
				...
				if(Stroke || IsStrokeCommand)
				{
					...
					else
					{
                        ...
                        if (pCommand->m_Flags&CMDFLAG_TEST && !g_Config.m_SvCheats)
                            m_Cheated = true;
					}
				}
			}
...
void CConsole::ConLevelCommand(IResult *pResult, void *pUser)
{
	...
	if(pCommand)
	{
		if(pResult->NumArguments() == 2)
		{
            ...
            pCommand->SetAccessLevel(
                pResult->GetInteger(1) == 0 ? ACCESS_LEVEL_USER :
                pResult->GetInteger(1) == 1 ? ACCESS_LEVEL_KID :
                pResult->GetInteger(1) == 2 ? ACCESS_LEVEL_HELPER :
            ...
            if(pResult->GetInteger(1) == 0) {
                pCommand->SetAccessLevel(ACCESS_LEVEL_USER); // ACCESS_LEVEL_USER
            }
