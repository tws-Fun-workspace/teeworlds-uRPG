...
int CServerBan::BanExt(T *pBanPool, const typename T::CDataType *pData, int Seconds, const char *pReason)
{
	...
	if(Server()->m_RconClientID >= 0 && Server()->m_RconClientID < MAX_CLIENTS &&
        ...
        Server()->m_aClients[Server()->m_RconClientID].m_State != CServer::CClient::STATE_DUMMY)
	{
...
void CServer::Kick(int ClientID, const char *pReason)
{
	...
	else if(m_aClients[ClientID].m_Authed > m_RconAuthLevel) // iDDRace64
	{
...
void CServer::KickForFlood(int ClientID, const char *pReason)
{
    m_ServerBan.BanAddr(m_NetServer.ClientAddr(ClientID), g_Config.m_SvFloodBan*60, "Flood");
}
/*int CServer::Tick()
{
...
const char *CServer::ClientName(int ClientID)
{
	if(ClientID < 0 || ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State == CServer::CClient::STATE_EMPTY)
		return "(invalid)";
	else if(m_aClients[ClientID].m_State == CServer::CClient::STATE_INGAME || m_aClients[ClientID].m_State == CServer::CClient::STATE_DUMMY) // iDDRace64
		return m_aClients[ClientID].m_aName;


	else
		return "(connecting)";
        // return false; // crash server
...
void CServer::ProcessClientPacket(CNetChunk *pPacket)
{
	...
	if(Sys)
	{
		...
		else if(Msg == NETMSG_RCON_CMD)
		{
			...
			if(Unpacker.Error() == 0 && m_aClients[ClientID].m_Authed)
			{
				...
				Console()->SetAccessLevel(
                    m_aClients[ClientID].m_Authed == AUTHED_ADMIN ? IConsole::ACCESS_LEVEL_ADMIN :
                    m_aClients[ClientID].m_Authed == AUTHED_MOD ? IConsole::ACCESS_LEVEL_MOD :
                    m_aClients[ClientID].m_Authed == AUTHED_HELPER ? IConsole::ACCESS_LEVEL_HELPER :
                    m_aClients[ClientID].m_Authed == AUTHED_KID ? IConsole::ACCESS_LEVEL_KID :
		...
		else if(Msg == NETMSG_RCON_AUTH)
		{
			...
			if(Unpacker.Error() == 0)
			{
				if(g_Config.m_SvRconPassword[0] == 0 && g_Config.m_SvRconModPassword[0] && g_Config.m_SvRconHelperPassword[0] == 0 && g_Config.m_SvRconKidPassword[0] == 0)
				{
                ...
                /*
                    ...
                    if(Unpacker.Error() == 0 && SendRconCmds) m_aClients[ClientID].m_pRconCmdToSend = Console()->FirstCommandInfo(IConsole::ACCESS_LEVEL_ADMIN, CFGFLAG_SERVER);
                ...
                */
                ...
                else if(g_Config.m_SvRconKidPassword[0] && str_comp(pPw, g_Config.m_SvRconKidPassword) == 0)
                {
                    ...
                    m_aClients[ClientID].m_Authed = AUTHED_KID;
                    int SendRconCmds = Unpacker.GetInt();
                    if(Unpacker.Error() == 0 && SendRconCmds)
                    ...
                    GameServer()->OnSetAuthed(ClientID, AUTHED_KID);
                }
...
int main(int argc, const char **argv) // ignore_convention
{
	...
	if(g_Config.m_SvDummies == 1)
		g_Config.m_SvMaxClients = MAX_CLIENTS;
	pEngine->InitLogfile();
...
void CServer::ConLogout(IConsole::IResult *pResult, void *pUser)
{
        ...
        pServer->m_aClients[pServer->m_RconClientID].m_State != CServer::CClient::STATE_EMPTY)
    {
        ...
        pServer->m_aClients[pServer->m_RconClientID].m_Authed = 0;
...
void CServer::SetRconLevel(int ClientID, int Level)
{
    if(Level == 0) {
        ...
        char *LevelName = "";
        if(Level == 1) {
            ConsoleAccessLevel = IConsole::ACCESS_LEVEL_KID;
        }
        else if (Level == 2) {
            ConsoleAccessLevel = IConsole::ACCESS_LEVEL_HELPER;
        }
        else if (Level == 3) {
            ConsoleAccessLevel = IConsole::ACCESS_LEVEL_MOD;
        }
        else {
            return;
        }
