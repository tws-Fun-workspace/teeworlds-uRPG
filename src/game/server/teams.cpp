...
void CGameTeams::OnFinish(CPlayer* Player)
{
    ...
    float timeStart = (float) (Server()->Tick() - GetStartTime(Player));
	if (time < 0.000001f)
		return;
	CPlayerData *pData = GameServer()->Score()->PlayerData(Player->GetCID());
	char aBuf[128];
	SetCpActive(Player, -2);
    if((int) time < g_Config.m_SvMinRecord) {
	...
	if (time - pData->m_BestTime < 0 && veryMinRecord == false)
	{
		// new record \o/
	...
	else if (pData->m_BestTime != 0  && veryMinRecord == false) // tee has already finished?
	{
    ...
    if(veryMinRecord) {
            GameServer()->SendChatTarget(Player->GetCID(), "Nice, but time won't be saved");
        }
	...
	if ((!pData->m_BestTime || time < pData->m_BestTime) && veryMinRecord == false)
	{
		// update the score
	...
	if (CallSaveScore && veryMinRecord == false)
	...
	if ((GameServer()->m_pController->m_CurrentRecord == 0
			|| time < GameServer()->m_pController->m_CurrentRecord) && veryMinRecord == false)
	{
		// check for nameless
		if (g_Config.m_SvNamelessScore || str_comp_num(Server()->ClientName(Player->GetCID()), "nameless tee",
				12) != 0)
	...
	// set player score
	if ((!pData->m_CurrentTime || pData->m_CurrentTime > time) && veryMinRecord == false)
	{
	...
	if (NeedToSendNewRecord && Player->m_IsUsingDDRaceClient && veryMinRecord == false)
	{
		for (int i = 0; i < MAX_CLIENTS; i++)
		{
...
void CGameTeams::SetSolo(int ClientID)
{
