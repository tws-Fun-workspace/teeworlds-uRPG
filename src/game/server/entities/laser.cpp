...
bool CLaser::HitCharacter(vec2 From, vec2 To)
{
    ...
    if(pHit != pOwnerChar) {
        if(m_Type == WEAPON_RIFLE) {
            pHit->TakeDamage(vec2(0.f, 0.f), GameServer()->Tuning()->m_LaserDamage*2, m_Owner, WEAPON_RIFLE);
        }
        else if (m_Type == WEAPON_SHOTGUN){
            pHit->TakeDamage(vec2(0.f, 0.f), GameServer()->Tuning()->m_LaserDamage*2, m_Owner, WEAPON_SHOTGUN);
        }
        else {
            //
        }
